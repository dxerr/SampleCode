...
void FPartsManager::Initialize(AMyCharacter* Character)
{
	Owner = Character;
	ActorComponent = Owner->FindComponentByClass<UActorComponent>();
}

void FPartsManager::LoadData(const TCHAR * Path)
{
	PartsFctory = LoadObject<UPartsContainer>(NULL, Path, NULL, LOAD_None, NULL);
	if (!PartsFctory)
	{
		UE_LOG(GameLog, Error, TEXT("파츠 데이터 로드에 실패하였습니다."));
	}
}

void FPartsManager::Attach(EPartsType Type, ...)
{
	const FPartsData* data = GetParts(Type);
	if (data)
	{
		TArray<USkeletalMesh*> parts;
		parts.Add(data->Mesh);
		USkeletalMesh* mesh = MergeParts(parts);
		if (mesh)
		{
...
void FPartsManager::Detach(EPartsType Type, ...)
{

}

const FPartsData* FPartsManager::GetParts(EPartsType Type)
{
	return PartsFctory->GetPartsData().FindByPredicate([=](const FPartsData& el)
	{
		return el.Type == Type;
	});
}

USkeletalMesh* FPartsManager::MergeParts(const TArray<USkeletalMesh*>& Parts) const 
{
	if (Parts.Num() > 0)
	{
		...
		for (int32 i = 0; i < Parts.Num(); i++)
		{
			...
			mergeMeshes.Add(Parts[i]);
		}

		if (mergeMeshes.Num() > 0)
		{
			USkeletalMesh* targetMesh = NewObject<USkeletalMesh>(ActorComponent, FName("MergedMesh"), RF_Transient);
...
void FPartsManager::ToMergeParams(const TArray<FSkelMeshMergeSectionMapping_BP>& InSectionMappings, TArray<FSkelMeshMergeSectionMapping>& OutSectionMappings)
{
...
void FPartsManager::ToMergeParams(const TArray<FSkelMeshMergeUVTransformMapping>& InUVTransformsPerMesh, TArray<FSkelMeshMergeUVTransforms>& OutUVTransformsPerMesh)
{
...
USkeletalMesh* FPartsManager::MergeToParams(const FSkeletalMeshMergeParams& Params)
{
