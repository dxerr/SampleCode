...
USkeletalMesh* USkeletalMeshMergeComponent::MergeToPieceMesh()
{
	if (PieceMeshes.Num() > 0)
	{
		...
		{
			if (PieceMeshes[i] == nullptr)
				continue;

			mergeMeshes.Add(PieceMeshes[i]);
		}
...
void USkeletalMeshMergeComponent::ToMergeParams(const TArray<FSkelMeshMergeUVTransformMapping>& InUVTransformsPerMesh, TArray<FSkelMeshMergeUVTransforms>& OutUVTransformsPerMesh)
{
...
USkeletalMesh* USkeletalMeshMergeComponent::MergeToParams()
{
	return MergeToParams(Params);
}

USkeletalMesh* USkeletalMeshMergeComponent::MergeToParams(const FSkeletalMeshMergeParams& Params)
{
