...
void FPartsLocal::Initialize(FBaseObject* owner)
{
	...
	Local = static_cast<ALocalPlayerObject*>(Owner);
	
	ActorComponent = Local->FindComponentByClass<UActorComponent>();
}


void FPartsLocal::Attach(EPartsType Type, ...)
{
	const FPartsDataBase* data = GetParts(Type);
	if (data)
	{
...
void FPartsLocal::Detach(EPartsType Type, ...)
{
	const FPartsDataBase* data = GetParts(Type);
	if (data)
	{
...
void FPartsLocal::ToMergeParams(const TArray<FSkelMeshMergeSectionMapping_BP>& InSectionMappings, TArray<FSkelMeshMergeSectionMapping>& OutSectionMappings)
{
...
void FPartsLocal::ToMergeParams(const TArray<FSkelMeshMergeUVTransformMapping>& InUVTransformsPerMesh, TArray<FSkelMeshMergeUVTransforms>& OutUVTransformsPerMesh)
{
...
USkeletalMesh* FPartsLocal::MergeToParams(const FSkeletalMeshMergeParams& Params)
{
